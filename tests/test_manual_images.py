...
class ManualImagesTests(unittest.TestCase):
    ...
    def test_manifest_contains_primary_and_refresh_prompts(self):
        ...
        self.assertIn("TrendForge Signal-Ink editorial illustration", manifest["entries"][0]["prompt"])
        self.assertIn("precise graphite and ink contours", manifest["entries"][0]["prompt"])
        self.assertIn("recurring coral signal thread", manifest["entries"][0]["prompt"])
        self.assertIn("never Japanese pastiche", manifest["entries"][0]["prompt"])
