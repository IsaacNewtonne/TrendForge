...
class ManualImagesTests(unittest.TestCase):
    ...
    def test_manifest_contains_primary_and_refresh_prompts(self):
        ...
        self.assertIn("Clean editorial infographic illustration", manifest["entries"][0]["prompt"])
        self.assertIn("soft retro-futurist isometric diagram look", manifest["entries"][0]["prompt"])
        self.assertIn("quiet magazine illustration", manifest["entries"][0]["prompt"])
        self.assertIn("not a UI, dashboard, labeled diagram, slide, or literal infographic", manifest["entries"][0]["prompt"])
